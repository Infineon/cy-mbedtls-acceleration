                          ...
                          const unsigned char *pk)
{
    ...
    volatile int ret = -1;
    volatile uint32_t stat_check = IFX_XMSS_SIG_INVALID;
    volatile int result = IFX_XMSS_SIG_INVALID;
    ...
    *mlen = smlen - params->sig_bytes;
    ...
    ret = memcmp(root, pub_root, params->n);
    /* First comparison - primary verification */
    if (ret == 0)
    {
        stat_check = ((uint32_t)IFX_XMSS_SIG_VALID & 0xFFFFu);
        verification_counter++;
    }
    else
    {
        stat_check = (uint32_t)IFX_XMSS_SIG_INVALID;
        /* zero the message */
        memset(m, 0, *mlen);
        *mlen = 0;
        return IFX_XMSS_SIG_INVALID;
    }

    if (verification_counter != 4U)
    {
        /* zero the message */
        memset(m, 0, *mlen);
        *mlen = 0;
	...
	if (ret == 0)
	{
		if (stat_check == ((uint32_t)IFX_XMSS_SIG_VALID & 0xFFFFu))
		{
			stat_check |= ((uint32_t)IFX_XMSS_SIG_VALID & 0xFFFF0000u);
			verification_counter++;
		}
	}
	else

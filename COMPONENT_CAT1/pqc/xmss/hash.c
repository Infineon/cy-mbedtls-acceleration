...
#if defined(CY_IP_MXCRYPTOLITE)
#define ifx_memcpy(dest,src,size) Cy_Cryptolite_Memcpy(CRYPTOLITE,(uint8_t*)dest,(uint8_t*)src,(uint32_t)size)
#elif (CY_IP_MXCRYPTO)
#define ifx_memcpy(dest,src,size) Cy_Crypto_Core_MemCpy(CRYPTO,(void*)dest,(void const*)src,(uint16_t)size)
#else 
#define ifx_memcpy(dest,src,size) memcpy((void*)dest,(const void*)src,(size_t)size)
#endif
                     ...
                     const unsigned char *in, unsigned long long inlen)
{
	...
	cy_stc_cryptolite_context_sha_t crp_cxt;
#else
	cy_stc_cryptolite_sha256_context_t crp_cxt;
#endif

    if (params->n == 24 && params->func == XMSS_SHA2) {
#if ((CY_IP_MXCRYPTOLITE_VERSION >=1) && (CY_IP_MXCRYPTOLITE_VERSION_MINOR >=2))
		Cy_Cryptolite_Sha_Run(CRYPTOLITE,CY_CRYPTOLITE_MODE_SHA256, in, inlen, buf, &crp_cxt);
#else
		Cy_Cryptolite_Sha256_Run(CRYPTOLITE, in, inlen, buf, &crp_cxt);
#endif
		ifx_memcpy(out, buf, 24);
	}
	else if (params->n == 32 && params->func == XMSS_SHA2) {
#if ((CY_IP_MXCRYPTOLITE_VERSION >=1) && (CY_IP_MXCRYPTOLITE_VERSION_MINOR >=2))
		Cy_Cryptolite_Sha_Run(CRYPTOLITE,CY_CRYPTOLITE_MODE_SHA256, in, inlen, out, &crp_cxt);
#else
		Cy_Cryptolite_Sha256_Run(CRYPTOLITE, in, inlen, buf, &crp_cxt);
#endif
	}
    ...
    if (params->n == 24 && params->func == XMSS_SHA2) {
        Cy_Crypto_Core_Sha(CRYPTO, in, inlen, buf, CY_CRYPTO_MODE_SHA256);
        Cy_Crypto_Core_MemCpy(CRYPTO, out, buf, 24);
    }
    else if (params->n == 32 && params->func == XMSS_SHA2) {
        Cy_Crypto_Core_Sha(CRYPTO, in, inlen, out, CY_CRYPTO_MODE_SHA256);
    }
#endif
    else {
        return -1;
        ...
        const unsigned char *key)
{
    ...
    ifx_memcpy(buf + params->padding_len, key, params->n);
    ifx_memcpy(buf + params->padding_len + params->n, in, 32);
        ...
        const unsigned char *key)
{
    ...
    ifx_memcpy(buf + params->padding_len, key, params->n);
    ifx_memcpy(buf + params->padding_len + params->n, in, params->n + 32);
                 ...
                 unsigned char *m_with_prefix, unsigned long long mlen)
{
    ...
    ifx_memcpy(m_with_prefix + params->padding_len, R, params->n);
    ifx_memcpy(m_with_prefix + params->padding_len + params->n, root, params->n);

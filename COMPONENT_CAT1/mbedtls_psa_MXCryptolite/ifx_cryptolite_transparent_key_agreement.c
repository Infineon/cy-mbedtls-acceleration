    ...
    size_t *shared_secret_length )
{
    ...
    mbedtls_ecp_group *ecp_grp = NULL;
    mbedtls_ecp_point *ecp_q = NULL;
    ...
    */
    ...
    if(shared_secret_size < bytesize)
    {
        ...
        static mbedtls_ecp_group ecp_buf;
        static mbedtls_ecp_point ecp_pt_buf;
        ...
        mbedtls_ecp_group ecp_buf;
        mbedtls_ecp_point ecp_pt_buf;
        ...
        ecp_grp = ifx_mxcryptolite_malloc(sizeof(mbedtls_ecp_group));
        ecp_q = ifx_mxcryptolite_malloc(sizeof(mbedtls_ecp_point));
    #endif

    if((ecp_grp != NULL) && (ecp_q != NULL))
    {
        ...
        psa_status = mbedtls_to_psa_error(mbedtls_ecp_group_load(ecp_grp, grp_id));
        if (psa_status != PSA_SUCCESS) {
            goto cleanup;
        }

        psa_status = mbedtls_to_psa_error(mbedtls_ecp_point_read_binary(ecp_grp, ecp_q,peer_key,peer_key_length));
        if (psa_status != PSA_SUCCESS) {
            goto cleanup;
        }

        psa_status = mbedtls_to_psa_error(mbedtls_ecp_check_pubkey(ecp_grp, ecp_q));
        if(psa_status != PSA_SUCCESS)
        {
           goto cleanup;
        }
    ...
    if( (NULL != ecdsa_buf_ptr) && (NULL != pubkey_ptr) && (NULL != pkey_ptr) && (NULL != ecp_grp) && (ecp_q != NULL))
    {
    ...
    cleanup:
        ...
        if(NULL != ecp_grp)
        {
            ifx_mxcryptolite_free(ecp_grp);
        }
        
        if(NULL != ecp_q)
        {
            ifx_mxcryptolite_free(ecp_q);
        }

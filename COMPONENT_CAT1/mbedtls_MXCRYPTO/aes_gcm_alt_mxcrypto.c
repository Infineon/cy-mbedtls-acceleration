...
void mbedtls_gcm_init(mbedtls_gcm_context *ctx)
{
    ...
    ctx->aes_state = (cy_stc_crypto_aes_gcm_state_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)ctx->aes_state_t);
    ctx->aes_buffers = (cy_stc_crypto_aes_gcm_buffers_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)ctx->aes_buffers_t);

#if (((CY_CPU_CORTEX_M7) && defined (ENABLE_CM7_DATA_CACHE)) || CY_CPU_CORTEX_M55)
    ctx->input_array_ptr  = (uint8_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)ctx->input_array);
                       ...
                       unsigned int keybits)
{
    ...
    {
#if (((CY_CPU_CORTEX_M7) && defined (ENABLE_CM7_DATA_CACHE)) || CY_CPU_CORTEX_M55)
		if( !CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)key, keybits) )
		{
			uint8_t *key_data = (uint8_t *)ifx_mbedtls_malloc(CY_CRYPTO_ALIGN_CACHE_LINE(keybits)+CY_CRYPTO_DCAHCE_PADDING_SIZE);
			if (NULL == key_data)
			{
				return AES_GCM_MEM_ALLOC_FAILED;
			}
			uint8_t *aligned_key_data = (uint8_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)key_data);

			ifx_mbedtls_memcpy((void *)aligned_key_data, (void *)key, keybits);
...
int mbedtls_gcm_starts(mbedtls_gcm_context *ctx, int mode, const unsigned char *iv, size_t iv_len)
{
    ...
    if(!CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)iv, iv_len))
    {
        ...
        uint8_t *aligned_iv_data = (uint8_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)iv_data);
...

{
    ...
    if( !CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)add, add_len) )
    {
        ...
        uint8_t *aligned_add_data = (uint8_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)add_data);
    ...
    *output_length = input_length;
	...
	if(!(CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)input, input_length)) || !(CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)output, *output_length)) )
	{
		...
		if(!(CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)input, input_length)))
		{
			...
			aligned_input_data = (uint8_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)input_data);
		}
		...
		if( !(CY_MBTLS_IS_MEM_CACHABLE_ALIGNED((uint32_t)output, *output_length)))
		{
			...
			aligned_output_data = (uint8_t*)CY_CRYPTO_DCAHCE_ALIGN_ADDRESS((size_t)output_data);
		}
                             ...
                             unsigned char *output)
{
    ...
    ifx_mbedtls_memcpy(ctx->output_array_ptr, check_tag, CY_CRYPTO_AES_BLOCK_SIZE);
    ifx_mbedtls_memcpy(ctx->input_array_ptr, tag, CY_CRYPTO_AES_BLOCK_SIZE);
